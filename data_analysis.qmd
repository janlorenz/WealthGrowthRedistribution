...
d <- read_parquet("simdata_v2/WealthGrowthRedistribution_StrippedDown experiments all.parquet") |> 
 mutate(Ns = format(N, scientific = FALSE, big.mark = ","),
        `Tax Regime` = case_when(tax_regime == "wealth" ~ "Wealth Tax", 
                                 tax_regime == "wealth gains" ~ "Wealth Gains Tax", 
                                 tax_regime == "realized wealth gains" ~ "Realized Wealth Gains Tax (1.5)"),
        # taxrate_wealth_scaled = if_else(tax_regime == "wealth", 7.5*taxrate, taxrate))
        taxrate_wealth_scaled_75 = if_else(tax_regime == "wealth gains", taxrate/7.5, taxrate),
        taxrate_wealth_scaled_60 = if_else(tax_regime == "wealth gains", taxrate/5.5 - 0.005, taxrate),
        taxrate_wealth_scaled_180 = if_else(tax_regime == "wealth gains", taxrate/18, taxrate),
        across(starts_with("tailexp"), \(x) x -1))
tax_regimes <- unique(d$`Tax Regime`)
N <- unique(d$N) |> sort()
Nss <- unique(d$Ns)
...
```{r}
d |> filter(base_taxrate_wealth > 0) |> 
 ggplot(aes(gini_stop_tick, stillintop10_past_tick_3, color = base_taxrate_wealth)) +
 geom_point(alpha = 0.1) + 
 geom_smooth(method = "lm") +
 facet_grid(allowance_fraction_median~`Tax Regime`) +
 labs(caption = "wealth tax regime, time lag 10") 
```
...
```{r}
d |> filter(base_taxrate_wealth > 0) |> 
 ggplot(aes(gini_stop_tick, stillintop10_past_tick_3, color = tax_regime, linetype = factor(allowance_fraction_median))) +
...
```{r}
d |> filter(base_taxrate_wealth > 0) |> 
 ggplot(aes(gini_stop_tick, stillintop10_past_tick_1, linetype = factor(allowance_fraction_median))) +
 geom_smooth(method = "lm") +
 labs(caption = "time lag 10")
```
