  ...
  mutate(Ns = format(N, scientific = FALSE, big.mark = ","), # renaming for plots
        `Tax Regime` = case_when(
         ...
         taxrate_wealth_scaled_1 = case_when(
          ...
          tax_regime == "wealth gains" ~ taxrate/7.5,
          tax_regime == "realized wealth gains" ~ taxrate/20),
         taxrate_wealth_scaled_2 = case_when(
          tax_regime == "wealth" ~ taxrate,
          tax_regime == "wealth gains" ~ taxrate/5.5 - 0.005,
          tax_regime == "realized wealth gains" ~ taxrate/10 - 0.01),
...
We hand-fitted two scaling factors such that outcome measures for different tax regimes coincide for different tax rates. To that end, we scaled down the tax rates of Wealth Gains Tax by a factor of 7.5 and those of Realized Wealth Gains (1.5) by a factor of 20. 

```{r}
#| fig-cap: Comparison of tax regimes with scales tax rates, version 1
d2 |> filter(N == 10000, stop_tick == 200) |> 
 ...
 labs(caption = 'scales: Wealth Tax as is, Wealth Gains tax scaled down by 7.5\nRealized Wealth Gains (1.5) scaled down by 20') +
 guides(color = guide_legend(position = "bottom", nrow = 1)) +
 theme_minimal()
```
...
For the critical value of Wealth Tax = 0.01 this implies critical values of Wealth Gains Tax = 0.075 and Realized Wealth Gains Tax (1.5) = 0.2.

For the value of Wealth Tax = 0.015 this implies comparable values of Wealth Gains Tax = 0.1125 and Realized Wealth Gains Tax (1.5) = 0.3
...
For the critical value of Wealth Tax = 0.01 this implies critical values of Wealth Gains Tax = 0.085 and Realized Wealth Gains Tax (1.5) = 0.2
